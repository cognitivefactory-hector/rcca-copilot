...
- **Selectivity tuned from live behavior.** The first production run was *correct* but verbose — the agent emitted a full 6M matrix (a dozen-plus causes, some near-duplicates). The prompt now caps D4 to ≤1 cause per category and ≤4 total, best-supported first, and says to omit a category rather than pad it with a speculative entry. Grounding is unchanged (still cite-or-flag). A focused, decision-useful cause list serves the engineer better than an exhaustive one — and reads better in the demo. Found only by watching the deployed agent, which is why the live smoke test exists.
